...
// & Constraints). "awards" is the next remaining stub id once Story 2.4
// makes "divisions" real, matching Story 2.3's own precedent of repointing
// this stub-proof test off the id it just made real.
func TestGetPredictSheetShouldRenderTheStubPageForAKnownSet(t *testing.T) {
	deadline := time.Now().UTC().Add(5 * 24 * time.Hour)
	seed := fmt.Sprintf(`    - id: awards
      title: Player awards
...
`, deadline.Format(time.RFC3339))
	...
	body := rec.Body.String()
	if !strings.Contains(body, "Player awards") {
...
// stub, never the cup/presidents/divisions closed read-only banner.
...
`, deadline.Format(time.RFC3339))
	...
	rec := postSheet(t, handler, "awards", "TOR")
