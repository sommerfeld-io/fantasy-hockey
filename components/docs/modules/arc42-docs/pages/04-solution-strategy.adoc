= Solution Strategy
:description: A short summary and explanation of the fundamental decisions and solution strategies, that shape system architecture and the way we work.
...
We prioritize running as many components as possible in Docker containers. This includes our own software applications, 3rd party tools used for development, and project dependencies. By containerizing these elements, we enhance portability, and consistency across different environments, while minimizing the requirements for development workstations. We package and ship our software using Docker containers.

== Test Driven Development
At our organization, we follow Test-Driven Development (TDD) principles, using TDD to design our systems.
...
Git tags are always created from the `main` branch, ensuring that they represent stable and release-worthy versions of the software. The creation of tags is done exclusively through our pipelines, guaranteeing consistent and reproducible release processes.
...
We get feedback on the quality of our work after every small change through the use of automated tests in our pipeline(s).
...
Manual tests are not forbidden. But since the only make us _feel_ better and more save instead of providing a real judgement on the releasability of our software, the make it or break it decision always comes from our pipeline(s).
...
While we may not release our software daily, our goal is to have the capability to release changes every day if required. To achieve this goal we have to keep our software in a releasable state all the time!
...
This pipeline(s) include all steps and tests that are sufficient to determine the releasability of our software. When a change has completed its transit through the pipeline(s), there is no more work to do for the software to be released into production other than triggering the release itself. Deployments are never done manually! This pipeline also documents this as a release.
